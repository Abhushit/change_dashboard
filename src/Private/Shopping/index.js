...
const Shopping = () => {
  ...
  const getAllShopping = () => {
    httpclient.get("shopping").then(({ data }) => {
      if (data.success) {
        setRows(data.data);
      }
    });
  ...
  const getPageVisible = () => {
    httpclient.get("shopping_visible").then(({ data }) => {
      if (data) {
  ...
  const handleSwitch = (e) => {
    ...
    httpclient
      .put(`/shopping_visible/${visibleID}`, {
  ...
  const handleAdd = () => {
    navigate(`/shopping/add_shopping`);
  };

  const handleEdit = (row) => {
    console.log("row", row);
    navigate(`/shopping/add_shopping/${row._id}`, { state: { row: row } });
  };
  ...
  const handleDeleteFinal = () => {
    httpclient.delete(`shopping/${selectedID}`).then(({ data }) => {
      if (data.success) {
        setOpen(true);
        setSnackStatus("success");
        setSnackMessage(data.message);
