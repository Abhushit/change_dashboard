...
const columns = [
  ...
  { id: "pageVisible", name: "Page Visible" },
...
const SinglePages = () => {
  ...
  const getAllSinglePage = () =>{
    httpclient.get("single_pages").then(({data}) => {
      if(data.success){
        setRows(data.data);
      }
    })
  ...
  const handleAdd = () => {
    navigate(`/single_pages/add_single_page`);
  };

  const handleEdit = (row) => {
    console.log("row", row);
    navigate(`/single_pages/add_single_page/${row._id}`, {
      state: { row: row },
    });
  };
  ...
  const handleDeleteFinal = () => {
    httpclient.delete(`single_pages/${selectedID}`).then(({data}) => {
      if(data.success){
        setOpen(true);
  ...
  const handleSearch = e => {
    // e.preventDefault();
    httpclient.post("single_pages/search",{
      search: searchText
    }).then(({data}) => {
      if(data.success){
        setRows(data.data);
