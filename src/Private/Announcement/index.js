...
const Announcement = () => {
  ...
  const getAllAnnouncements = () => {
    httpclient.get("announcements").then(({ data }) => {
      if (data.success) {
        setRows(data.data);
      }
    });
  ...
  const handleAdd = () => {
    navigate(`/announcements/add_announcement`);
  };

  const handleEdit = (row) => {
    navigate(`/announcements/add_announcement/${row._id}`, {
      state: { row: row },
    });
  };
  ...
  const handleDeleteFinal = () => {
    httpclient.delete(`announcements/${selectedID}`).then(({ data }) => {
      if (data.success) {
        setOpen(true);
